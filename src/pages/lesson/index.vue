  ...
  <div>
    <div class="optioncardbox">
      ...
      <div class="optioncard optioncardlist" v-for="(item,index) in lessonlisttype" :key="index">
        <img
          :src="[cur==index?item.icon:item.ico]"
          alt
          :class="[cur==index?d:c]"
    ...
    <div class="lessonlist">
      <div class="lessonlisthead">
        ...
        <div class="lessonlistoption">
          <span
            :class="[tapcur==index?o:p]"
            @click="changetip(index)"
            v-for="(item,index) in lessontaplist"
            :key="index"
          >{{item.title}}</span>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    // 前往课程列表
    GoLessonPage(x, y) {
      this.$store.commit("GetLessonlistContent", this.goodsinfo[x]);
      wx.navigateTo({
