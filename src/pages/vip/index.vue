<template>
  <div class="vip">
    <!-- 头部 -->
    <div class="vip-head">
      <div class="vip-head-title">VIP会员专享
        <div style="font-size:12px;margin-top:10px;" v-if="isvip">您的会员将于：{{time}}到期</div>
      </div>
      <block v-if="isvip == false">
        <div style="display:flex;position:relative;">
          <button
            style="margin-bottom:0px;width:40%;height:40px;line-height:40px;float:left;margin-top:20px;background:#616161;border:none;color:#fff;font-size:20px;position:absolute;bottom:-20px;left:25px;background: linear-gradient(to right, #ec881d, rgb(227, 200, 39));"
            @click="GoExchange"
          >VIP兑换</button>
          <button
            plain="true"
            style="margin-bottom:0px;width:40%;height:40px;line-height:40px;float:left;margin-top:20px;background:#616161;border:none;color:#fff;font-size:20px;position:absolute;bottom:-20px;right:25px;background: linear-gradient(to right, #ec881d, rgb(227, 200, 39));"
            @click="buyshow=true"
          >VIP查询</button>
        </div>
      </block>
    ...
    <div class="buylinkbox" :class="[buyshow?s:h]">
      <div class="buylink">
        <div class="buyclose" @click="buyshow=false">X</div>
        ...
        <div class="buychoose">
          <div
            class="choosecom chooseleft"
            :class="[priceischoose==0?a:n]"
            @click="ChangePrice(0)"
          >￥{{price1}}元/</div>
          <!-- <div class="choosemin"></div> -->
          <div
            class="choosecom chooseright"
            :class="[priceischoose==1?a:n]"
            @click="ChangePrice(1)"
          >￥{{price2}}元/</div>
        </div>
        <div class="buyfoot">
...
export default {
  data() {
    return {
      ...
      priceischoose: 0,
      link:
        '{"type": "vip","userid":"null","unionId":"null","goodsid":"1","price":"null"}',
      s: "show",
      h: "hide",
      a: "active",
      n: "normal",
