      ...
      <div class="add_input">
        ...
        <div class="input_box">
          ...
          <picker
            ...
            class="input_box_input"
            :value="childrenBirthday"
            start="1990-01-01"
            end="2020-09-01"
            @change="ChangeChildrenBirthday"
          >
        ...
        <div class="input_box">
          ...
          <picker
            @change="ChangeChildrenHight"
            :value="childrenHightindex"
            :range="childrenHightlist"
            class="input_box_input"
          >
            <view class="picker">当前选择：{{childrenHightlist[childrenHightindex]}}厘米</view>
          </picker>
        </div>
        <div class="input_box">
          <p class="input_box_p">目前身高</p>
          <picker
            @change="ChangeChildrenHightNow"
            :value="childrenHightnowIndex"
            :range="childrenHightlist"
            class="input_box_input"
          >
            <view class="picker">当前选择：{{childrenHightlist[childrenHightnowIndex]}}厘米</view>
          </picker>
        </div>
        <div class="input_box" v-if="isParshow">
          <p class="input_box_p">父亲身高</p>
          <picker
            @change="ChangeFatherHightNow"
            :value="fatherHightnowIndex"
            :range="childrenHightlist"
            class="input_box_input"
          >
            <view class="picker">当前选择：{{childrenHightlist[fatherHightnowIndex]}}厘米</view>
          </picker>
        </div>
        <div class="input_box" v-if="isParshow">
          <p class="input_box_p">母亲身高</p>
          <picker
            @change="ChangeMotherHightNow"
            :value="motherHightnowIndex"
            :range="childrenHightlist"
            class="input_box_input"
          >
            <view class="picker">当前选择：{{childrenHightlist[motherHightnowIndex]}}厘米</view>
          </picker>
        </div>
      </div>
...
import { addChildrenUrl, isParUrl } from "@/utils/api.js";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    // 计算数值
    ComputeNumber(x) {
      return JSON.parse(x) + 100;
    },

    // 计算年龄
    ComputeAge(x) {
      this.age = new Date().getFullYear() - parseInt(x.slice(0, 4));
    },
    // 显示提示
    showMessage(x) {
      wx.showToast({
    ...
    GoTest() {
      ...
      if (this.checked == null) {
        ...
        ajax
          .Get(`${addChildrenUrl}${this.$store.state.userParam.openId}`, {
            sex: this.checked,
            username: this.name,
    ...
    GetDefaultState() {
      ajax
        .Post(isParUrl, { openid: this.$store.state.userParam.openId })
        .then(res => {
          res.result == "true"
